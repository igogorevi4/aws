...
def main_logic():
    for user in resource.users.all():
        check_user_not_service_admin(user)


def check_user_not_service_admin(user):
    user_tags = client.list_user_tags(UserName=user.name)
    is_user_service_admin = False
    for tags in user_tags['Tags']:
        if tags['Key'] == 'admin' and tags['Value'] == 'service':
            is_user_service_admin = True
    if not is_user_service_admin:
